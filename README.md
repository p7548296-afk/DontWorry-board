...
- 이메일 셀렉트 방식 회원가입 / 로그인 / 로그아웃 (닉네임 중심 식별)
- 비밀번호 정책 적용 (8자 이상, 대소문자/숫자/특수문자 포함) 및 이중 확인
- 게시글 작성 / 수정 / 삭제 / 상세 조회 (이미지 최대 5장 첨부)
- 댓글 작성 / 수정 / 삭제 / 조회
- 게시글 목록 (이미지 썸네일 + 댓글 수 표시)
- 게시글 검색 (제목/내용)
- 게시글/댓글 페이지네이션
- 이미지 라이트박스 (크기 확대, 개수 표시, 좌우 이동)
- 페이지 전환 로딩 UI (Suspense 스켈레톤)
- 로그인 사용자 닉네임 표시
...
## 문제 2: 이미지 라이트박스 Hydration 에러

### 문제 상황

이미지가 있는 게시글 상세 페이지 진입 시 React Hydration 에러 발생

### 원인

이미지 배열을 `map()`으로 순회하여 Lightbox를 여러 개 렌더링할 때 서버/클라이언트 간 `aria-controls` ID 불일치 발생

### 해결 방법

Lightbox를 개별로 여러 개 렌더링하는 방식 대신 `PostImageGallery` 컴포넌트를 신규 생성하여 하나의 Lightbox 인스턴스로 통합 관리

---

# 회고

- **어려웠던 점:** Vercel 배포 과정에서 한글 경로로 인한 Turbopack 빌드 오류 해결, 라이트박스 Hydration 에러 디버깅
- **개선하고 싶은 점:** 마이페이지, 반응형 모바일 UI 등 고도화 기능 추가
- **새롭게 배운 점:** Next.js App Router의 Server Component / Server Action 패턴, Supabase RLS 설정, React Hydration 에러 원인 및 해결 방법
- **AI 에이전트를 사용하며 느낀 점:** 문서 기반으로 컨텍스트를 유지하면 AI가 일관된 코드를 생성할 수 있음. 구체적인 프롬프트일수록 원하는 결과를 빠르게 얻을 수 있음
